...
for(i in 1:n_rep){
  ...
  nonzero <- rbinom(p2,1,sparsityG)
  if(sum(nonzero)==0) nonzero[1] <- 1
  alpha <- as.matrix(rnorm(p2,0,sqrt(sg2_true/t*p2/sum(nonzero))))
  alpha[nonzero==0] <- 0
  ...
  out[i,7:8] <- REML$H[2,1:2]

  # exact estimate by MoM
  MoM <- MoM_3var(K,Ka,z)
  out[i,3:4] <- MoM$H[1,1:2]
  out[i,9:10] <- MoM$H[2,1:2]
...
write.table(out,file=paste("sparsityB",sparsityB,"_sparsityG",sparsityG,"_SNRy",sb2_true,"_n",n1,"_",n2,".txt",sep=""),quote = F,col.names = T,row.names = F)
